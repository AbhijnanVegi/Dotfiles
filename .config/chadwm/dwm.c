...
void focus(Client *c) {
  if (!c || (!ISVISIBLE(c,selmon) || HIDDEN(c)))
    for (c = selmon->cl->stack; c && (!ISVISIBLE(c, selmon) || HIDDEN(c)); c = c->snext)
      ;
...
void focusstack(const Arg *arg) {
  ...
  if (!selmon->sel || (selmon->sel->isfullscreen && lockfullscreen))
    return;
  if (arg->i > 0) {
    for (c = selmon->sel->next; c && (!ISVISIBLE(c,selmon) || HIDDEN(c)); c = c->next)
      ;
    if (!c)
      for (c = selmon->cl->clients; c && (!ISVISIBLE(c,selmon) || HIDDEN(c)); c = c->next)
        ;
  } else {
    for (i = selmon->cl->clients; i != selmon->sel; i = i->next)
      if (ISVISIBLE(i, selmon) && !HIDDEN(i))
        c = i;
    if (!c)
      for (; i; i = i->next)
        if (ISVISIBLE(i, selmon) && !HIDDEN(i))
...
focuswin(const Arg* arg){
	int iwin = arg->i;
	Client* c = NULL;
	for(c = selmon->cl->clients; c && (iwin || !ISVISIBLE(c,selmon)) ; c = c->next){
		if(ISVISIBLE(c,selmon)) --iwin;
	};
	if(c) {
		focus(c);
		restack(selmon);
...
Client *nexttiled(Client *c, Monitor* m) {
  for (; c && (c->isfloating || (!ISVISIBLE(c,m) || HIDDEN(c))); c = c->next)
    ;
  return c;
}
...
togglescratch(const Arg *arg)
{
	...
	for (c = selmon->cl->clients; c && !(found = c->scratchkey == ((char**)arg->v)[0][0]); c = c->next);
	if (found) {
		c->tags = ISVISIBLE(c, selmon) ? 0 : selmon->tagset[selmon->seltags];
		focus(NULL);
		arrange(selmon);
